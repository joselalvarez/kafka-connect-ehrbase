...
@Slf4j
public class CompositionEventMapper {

    private final ObjectMapper canonicalObjectMapper;
    private final OpenEHRSourceConnectorConfig connectorConfig;

    public Struct mapStruct(CompositionChange source) {
        CompositionEvent target = new CompositionEvent();

        target.setEventType(source.getChangeType().getValue());
        ...
        target.setContributionId(source.getContributionId().toString());
    ...
    public SourceRecord mapRecord(CompositionChange source) {
        return new SourceRecord(
                ...
                CompositionEvent.SCHEMA, // Value schema
