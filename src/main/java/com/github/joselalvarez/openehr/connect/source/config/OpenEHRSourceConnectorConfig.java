...
public class OpenEHRSourceConnectorConfig extends ConnectorConfig {
    ...
    public ConfigDef definitions() {
        ...
        def.define(KAFKA_COMPOSITION_TOPIC, ConfigDef.Type.STRING, null, ConfigDef.Importance.HIGH, "Kafka topic where composition change events are published");
        def.define(KAFKA_EHR_TOPIC, ConfigDef.Type.STRING, null, ConfigDef.Importance.HIGH, "Kafka topic where EHR status change events are published");
        def.define(POLL_INTERVAL_MS, ConfigDef.Type.LONG, POLL_INTERVAL_MS_DEF, ConfigDef.Importance.MEDIUM, "Minimum time between polling cycles in milliseconds");
        def.define(POLL_BATCH_SIZE, ConfigDef.Type.LONG, POLL_BATCH_SIZE_DEF, ConfigDef.Importance.MEDIUM, "Maximum number of records retrieved per poll");

        def.define(FILTER_COMPOSITION_FROM_DATE, ConfigDef.Type.STRING, null, dateTimeValidator, ConfigDef.Importance.LOW, "Start date from which compositions are retrieved (ISO 8601 format)", COMPOSITION_FILTER_GROUP, -1, ConfigDef.Width.NONE, FILTER_COMPOSITION_FROM_DATE);
        def.define(FILTER_COMPOSITION_TO_DATE, ConfigDef.Type.STRING, null, dateTimeValidator, ConfigDef.Importance.LOW, "End date until which compositions are retrieved (ISO 8601 format)", COMPOSITION_FILTER_GROUP, -1, ConfigDef.Width.NONE, FILTER_COMPOSITION_TO_DATE);
        def.define(FILTER_COMPOSITION_BY_TEMPLATE, ConfigDef.Type.STRING, null, (ConfigDef.Validator) null, ConfigDef.Importance.LOW, "Template ID of compositions to retrieve", COMPOSITION_FILTER_GROUP, -1, ConfigDef.Width.NONE, FILTER_COMPOSITION_BY_TEMPLATE);
        def.define(FILTER_COMPOSITION_BY_ARCHETYPE, ConfigDef.Type.STRING, null, archetypeValidator, ConfigDef.Importance.LOW, "Archetype ID of compositions to retrieve", COMPOSITION_FILTER_GROUP, -1, ConfigDef.Width.NONE, FILTER_COMPOSITION_BY_ARCHETYPE);

        def.define(FILTER_ERH_STATUS_FROM_DATE, ConfigDef.Type.STRING, null, dateTimeValidator, ConfigDef.Importance.LOW, "Start date from which EHR status entries are retrieved (ISO 8601 format)", ERH_STATUS_FILTER_GROUP, -1, ConfigDef.Width.NONE, FILTER_ERH_STATUS_FROM_DATE);
        def.define(FILTER_ERH_STATUS_TO_DATE, ConfigDef.Type.STRING, null, dateTimeValidator, ConfigDef.Importance.LOW, "End date until which EHR status entries are retrieved (ISO 8601 format)", ERH_STATUS_FILTER_GROUP, -1, ConfigDef.Width.NONE, FILTER_ERH_STATUS_TO_DATE);
